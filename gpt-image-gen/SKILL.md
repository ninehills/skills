---
name: gpt-image-gen
description: "生图 / 生成图片 / 画图 — 用 OpenAI gpt-image-2 生成图像。支持文生图、参考图生图 (img2img)、蒙版修补 (inpainting)。当用户要求用 GPT 画图、OpenAI 生图、gpt-image-2、文+图生图、参考图片生成、img2img、inpainting 时必加载此技能。Auth 自动继承 OPENAI_API_KEY / Codex token / .env / config.yaml。"
version: 2.0.0
author: Hermes Agent
license: MIT
metadata:
...
2. `./.env` 文件（当前目录）
3. `~/.env` 文件（家目录）
4. Codex OAuth token（`hermes auth codex` 登录过的）
5. `config.yaml` → `image_gen.openai.api_key`
6. `--api-key` 手动传入

**⚠️ Codex token 限制：** Codex OAuth token 只能认证 `chatgpt.com/backend-api/codex`，**不能**直连 `api.openai.com`。如果 auth 解析落到 Codex token（优先级 4），脚本会拿它当标准 API key 去打 `api.openai.com/v1/images/*`，返回 **401 Missing scopes: api.model.images.request**。文生图请用 Hermes 内置 `image_generate` 工具（走 Codex backend）；img2img / inpainting（`images.edit`）必须有真正的 `OPENAI_API_KEY`。详见 `references/auth-pitfalls.md`。
